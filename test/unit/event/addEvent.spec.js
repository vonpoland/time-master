...
describe('events tests', function () {
    ...
    it('should not throw error when TimeEvent is passed', function () {
        let timeMaster = new TimeMaster();
        let timeEvent = TimeMaster.createEvent('event', new Date(), () => {});
        let wasError = false;
    ...
    it('should run events in sequence', function (done) {
        let timeMaster = new TimeMaster();
        let order = [];
        let timeEvent1 = TimeMaster.createEvent('name1', moment().add(0.02, 'second'), () => order.push(timeEvent1.name));
        let timeEvent2 = TimeMaster.createEvent('name2', moment().add(0.03, 'second'), () => order.push(timeEvent2.name));
        let timeEvent3 = TimeMaster.createEvent('name3', moment().add(0.04, 'second'), () => order.push(timeEvent3.name));
        let timeEvent4 = TimeMaster.createEvent('name4', moment().add(0.05, 'second'), () => order.push(timeEvent4.name));
        let timeEvent5 = TimeMaster.createEvent('name5', moment().add(0.06, 'second'), () => order.push(timeEvent5.name));
